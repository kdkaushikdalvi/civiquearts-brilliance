...
interface Props {
  ...
  options: Option[];
  placeholder?: string;
  onEmptyAction?: (query: string) => void | Promise<void>;
}
        ...
        >
          {selected.length
            ? selected.map((option) => (
                <span
                  key={option.id}
                  className="inline-flex max-w-full items-center gap-1 rounded-full bg-secondary px-2 py-0.5 text-xs"
                >
        ...
        <div className="absolute top-full left-0 z-[100] mt-1 w-full min-w-[240px] overflow-hidden rounded-md border border-border bg-popover shadow-2xl">
          ...
          <div className="max-h-56 overflow-y-auto p-1">
            {filtered.length ? (
              ...
              <div>
                {query.trim() && onEmptyAction ? (
                  <button
                    type="button"
                    onClick={() => {
                      setOpen(false);
                      setQuery("");
                      onEmptyAction(query.trim());
                    }}
                  ...
                  >
                    <Plus className="h-4 w-4" /> Add Assignee “{query.trim()}”
                  </button>
                ) : (
