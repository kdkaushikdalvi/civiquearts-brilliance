...
interface Props { value: string[]; onChange: (ids: string[]) => void; options: Option[]; placeholder?: string; onEmptyAction?: (query: string) => void | Promise<void> }

const MultiSearchableSelect = ({ value, onChange, options, placeholder = "Select Assignee", onEmptyAction }: Props) => {
  ...
  return <div ref={wrap} className="relative">
    ...
    {open && <div className="absolute bottom-full z-50 mb-1 w-full min-w-[220px] overflow-hidden rounded-md border border-border bg-popover shadow-lg">
      ...
      <div className="max-h-56 overflow-y-auto p-1">{filtered.length ? filtered.map((option) => { const checked = selectedIds.includes(option.id); return <button key={option.id} type="button" onClick={() => toggle(option.id)} className="flex w-full items-center gap-2 rounded px-2 py-2 text-left text-sm hover:bg-secondary"><span className={cn("flex h-4 w-4 items-center justify-center rounded-sm border", checked && "border-primary bg-primary text-primary-foreground")}>{checked && <Check className="h-3 w-3" />}</span>{option.label}</button>; }) : <div>{query.trim() && onEmptyAction ? <button type="button" onClick={() => { setOpen(false); setQuery(""); onEmptyAction(query.trim()); }} className="flex w-full items-center justify-center gap-2 p-3 text-sm text-saffron transition-colors hover:bg-saffron hover:text-white"><Plus className="h-4 w-4" /> Add Assignee “{query.trim()}”</button> : <div className="p-3 text-center text-sm text-muted-foreground">No assignees found</div>}</div>}</div>
    </div>}
  </div>;
};
